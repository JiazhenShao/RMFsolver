...
# pressure of a single fermion species
def _P_f(mu, m, Tem, upB=np.inf):
    '''
    ...
    if Tem > 1e-3:
        def integrand(k):
            Ek = np.sqrt(k**2 + m**2)
            fd = 1/(1 + np.exp(np.clip((Ek - mu)/Tem, -700, 700)))
            fd_bar = 1/(1 + np.exp(np.clip((Ek + mu)/Tem, -700, 700)))
            return (fd + fd_bar) * k**4 / (3 * Ek)
...
def n_electron(T, muQ, me=0.511, upB=np.inf):
    """
    ...
    """
    return _n_f(mu=muQ, m=me, Tem=T, upB=upB)
...
def electron_pressure(T, muQ, me=0.511, upB=np.inf, method):
    """
    ...
    upB    : upper bound for integration (MeV)
    method : "Adaptive" 
             "ibp" - integral by parts

    Returns:
    float : pressure (MeV^4)
    """
    if method == "ibp":
        return _P_f(mu=muQ, m=me, Tem=T, upB=upB)
