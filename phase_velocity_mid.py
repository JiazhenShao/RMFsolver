...
# Public functions
__all__ = ["P_f", "E_f", "n_B", "nQM", "nQM_em", "PQM", "PQM_em", "edensQM", "vNtoQ_Pc", "vNtoQ_B", "vNtoQ_nc", "Get_Delta_n_max",
...
def _nm_pressure_muB(mu_B, Temp, param, NM_type):
    """
...
def _nm_pressure_nB(nB, Temp, param, NM_type):
    """
...
def nQM(muB, muK, B_one_forth, T, ms=0, upB=5000, return_species=False):
    """
...
def _find_muB_muK_star(PQM, P_target, C, B_one_forth, T, ms, upB=5000, initial_guess=(1020.0, 700.0)):
    ...
    def dPQM_dmuB(muB, muK):
        return nQM(muB, muK, B_one_forth, T, ms=ms, upB=upB)
...
def vNtoQ_Pc(T, P_crit, DelP, m_s, param, NM_type, method="analytical", aQmax=True):
    ...
    P_diff = lambda x: _nm_pressure_muB(x, T, param, NM_type) - P_crit
    ...
    PNM_minus_PShift = lambda x: _nm_pressure_muB(x, T, param, NM_type) - P_crit - DelP
    ...
    # calculating aN
    PNM_wrap = lambda x: _nm_pressure_muB(x, T, param, NM_type)
...
def vNtoQ_Pc_fixB(T, P_crit0, DelP, m_s, param, NM_type, method="analytical", aQmax=True):
    ...
    # solve for bag constant
    muB_crit0 = fsolve(lambda x: _nm_pressure_muB(x, Temp=0, param=param, NM_type=NM_type) - P_crit0, 1050)[0]
    PQM_solve_for_B = lambda x: PQM(muB_crit0, 0, x, 0, m_s) - P_crit0
    B_SQM = fsolve(PQM_solve_for_B, 180)[0]
    muB_crit = fsolve(lambda x: _nm_pressure_muB(x, T, param, NM_type) - PQM(x, 0, B_SQM, T, m_s), 1050)[0]
...
def vNtoQ_B(T, B_SQM, DelP, m_s, param, NM_type, method="analytical", aQmax=True):
    ...
    P_diff = lambda x: PQM(x, 0, B_SQM, T, m_s) - _nm_pressure_muB(x, T, param, NM_type)
    muB_crit = fsolve(P_diff, 1050)[0]
    P_crit = PQM(muB_crit, 0, B_SQM, T, m_s)
...
def vNtoQ_nc(T, n_crit, Deln, m_s, param, NM_type, method="analytical", aQmax=True):
    ...
    # solve for bag constant
    P_crit = _nm_pressure_nB(n_crit, T, param, NM_type)
    P_diff = lambda x: _nm_pressure_muB(x, T, param, NM_type) - P_crit
    muB_crit = fsolve(P_diff, 1050)[0]
    PQM_solve_for_B = lambda x: PQM(muB_crit, 0, x, T, m_s) - P_crit
    B_SQM = fsolve(PQM_solve_for_B, 180)[0]
    ...
    # solve for points Q and N at beta eq.
    DelP = _nm_pressure_nB(n_crit + Deln, T, param, NM_type) - P_crit
    PNM_minus_PShift = lambda x: _nm_pressure_muB(x, T, param, NM_type) - P_crit - DelP
    muB_N = fsolve(PNM_minus_PShift, 1050)[0]
...
def vNtoQ_fixB(T, n_crit, Deln, m_s, param, NM_type, method="analytical", aQmax=True):
    ...
    # solve for bag constant
    P_crit0 = _nm_pressure_nB(n_crit, Temp=0, param=param, NM_type=NM_type)
    muB_crit0 = fsolve(lambda x: _nm_pressure_muB(x, Temp=0, param=param, NM_type=NM_type) - P_crit0, 1050)[0]
    PQM_solve_for_B = lambda x: PQM(muB_crit0, muK=0, B_one_forth=x, T=0, ms=m_s) - P_crit0
    B_SQM = fsolve(PQM_solve_for_B, 180)[0]

    # solve for critical point
    muB_crit = fsolve(lambda x: PQM(x, 0, B_SQM, T, m_s) - _nm_pressure_muB(x, T, param, NM_type), 1050)[0]
    P_crit = PQM(muB_crit, 0, B_SQM, T, m_s)
    n_crit_T = fsolve(lambda x: _nm_pressure_nB(x, T, param, NM_type) - P_crit, 2*n0)[0]
    DelP = _nm_pressure_nB(n_crit_T + Deln, T, param, NM_type) - P_crit
...
def Get_Delta_n_max(T, n_crit, m_s, param, NM_type, aQmax=True, tol=5e-3, coarse_pts=5, bounds=None):
    ...
    muB_crit = fsolve_fast(lambda x: _nm_pressure_muB(x, T, param, NM_type) - P_crit, 1050.0)
    ...
    def Get_aN_aQstar_diff(Delta_n):
        ...
        DelP = _nm_pressure_nB(n_crit + Delta_n, T, param, NM_type) - P_crit

        muB_N = fsolve_fast(lambda x: _nm_pressure_muB(x, T, param, NM_type) - P_crit - DelP, 1050.0)
...
def z_time_evolution3(dn_over_dr, n_crit0, T, z0 = 1, t_up=20, Interp1d_Num=10, m_s=0, param = para.paraQMCRMF3, NM_type = "PNM", aQmax=True):
    ...
    # 1) calculate P_critical. We define it to be whatever pressure 1km away from the kink
    P_crit0 = _nm_pressure_nB(n_crit0, Temp=0, param=param, NM_type=NM_type)
    muB_crit0 = fsolve(lambda x: _nm_pressure_muB(x, Temp=0, param=param, NM_type=NM_type) - P_crit0, 1050)[0]
    PQM_solve_for_B = lambda x: PQM(muB_crit0, muK=0, B_one_forth=x, T=0, ms=m_s) - P_crit0
    B_SQM = fsolve(PQM_solve_for_B, 180)[0]
    ...
    muB_crit = fsolve(lambda x: PQM(x, 0, B_SQM, T, m_s) - _nm_pressure_muB(x, T, param, NM_type), 1050)[0]
    P_crit = PQM(muB_crit, 0, B_SQM, T, m_s)
    n_crit = fsolve(lambda x: _nm_pressure_nB(x, T, param, NM_type) - P_crit, 3*n0)[0]
