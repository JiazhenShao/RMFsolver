...
def _log1p_exp_neg(a):
    """
    Numerically stable evaluation of log(1 + exp(-a)) for scalar a.
    """
    a = float(np.clip(a, -700.0, 700.0))  # avoid insane overflows in exp
    if a > 40.0:
        # exp(-a) is tiny, log(1 + tiny) ~ tiny
        return np.exp(-a)
    elif a < -40.0:
        # exp(-a) huge, 1 + exp(-a) ~ exp(-a) => log ~ -a
        return -a
    else:
        return np.log1p(np.exp(-a))
def nucleon_pressure_fixed(T, mu, sigma, w0, r03, BI):
    """
    Pressure of a baryon species in RMF.
    ...
    # ---- T ~ 0 branch: analytic zero-T RMF expression ----
    if T <= 1.0e-3:
        kf = _kf_bar(sigma, w0, r03, mu, BI)
        Ef = _Ef_bar(sigma, w0, r03, mu, BI)
        if (m_eff <= 0.0) or ((kf + Ef) <= 0.0):
            return 0.0
        term1 = (2.0/3.0 * kf**3 - m_eff*m_eff * kf) * Ef
        term2 = m_eff**4 * np.log((kf + Ef) / max(m_eff, 1e-300))
        return (term1 + term2) / (8.0 * np.pi**2)

    # ---- finite-T branch: pressure integral with ONLY the log1p fix ----
    def integrand(k):
        E = np.sqrt(k*k + m_eff*m_eff)
        a = (E - mu_eff) / T
        b = (E + mu_eff) / T
        pos = _log1p_exp_neg(a)
        neg = _log1p_exp_neg(b)
        return k*k * (pos + neg)

    # upB is your global momentum cutoff (same as before)
    ub = np.inf if (upB is None or not np.isfinite(float(upB))) else float(upB)

    result, _ = quad(integrand, 0.0, ub, epsabs=1e-10, epsrel=1e-8, limit=200)
    # same overall prefactor as your original code:
    # 2*T * 4π / (2π)^3 = T / π^2
    return 2.0 * T * 4.0 * np.pi / (2.0 * np.pi)**3 * result
def nucleon_pressure_sommerfeld(T, mu, sigma, w0, r03, BI):
    """
    Pressure of a baryon species in RMF using ONLY the Sommerfeld expansion
    (valid for low temperatures, degenerate regime).
    ...
    -------
    float
        Pressure (MeV^4), approximated by P0 + (π²/6) T² fP.
    """
    T      = float(T)
    m_eff  = float(mB[BI] - gs[BI] * sigma)
    mu_eff = float(mu    - gw[BI] * w0 - gr[BI] * I3[BI] * r03)

    # If below threshold, no Fermi surface → zero pressure
    ...
    # Exact T=0 RMF pressure (same as in zero-T branch of nucleon_pressure)
    term1 = (2.0/3.0 * kF**3 - m_eff*m_eff * kF) * Ef
    term2 = m_eff**4 * np.log((kF + Ef) / max(m_eff, 1e-300))
    ...
    # Sommerfeld coefficient fP(μ_eff, m_eff)
    # P(T) ≈ P0 + (π²/6) T² fP,  with fP = μ_eff kF / π²
    fP = (mu_eff * kF) / (np.pi**2)

    return P0 + (np.pi**2 / 6.0) * (T*T) * fP
