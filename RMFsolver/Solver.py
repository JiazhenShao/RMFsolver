...
def baryon_density(T, muB, BI, sigma, w0, r03, para, up=5000):
    """
    Public wrapper around `_nB` that sets model couplings.
    ...
    """
    _set_couplings(para)
    return _nB(T, muB, BI, sigma, w0, r03, upB=up)

def _species_baryon_densities_from_soltab(soltab):
    """
    Return total and species baryon densities from an RMF solution table.
    """
    para = soltab[4]
    _set_couplings(para)

    T = soltab[2]
    sigma, w0, r03 = soltab[0][:3]
    mu_n = soltab[0][3]
    mu_p = soltab[0][4] if len(soltab[0]) > 4 and isinstance(soltab[0][4], (int, float, np.floating)) else 0.0

    n_n = _nB(T, mu_n, 0, sigma, w0, r03)
    n_p = _nB(T, mu_p, 1, sigma, w0, r03) if mu_p > 0 else 0.0
    return n_n + n_p, n_n, n_p

def _solve_from_input_type(input_num, input_type, solve_from_nb, solve_from_mub):
    """
    Shared dispatcher for wrappers supporting input_type in {"nB", "muB"}.
    """
    if input_type == "nB":
        return solve_from_nb(input_num)
    if input_type == "muB":
...
def electron_edens(T, muQ, me=0.511, upB=5000, Sommerfeld=False):
    """
    Free electron energy density at finite or zero temperature.
    ...
    me         : electron mass (MeV)
    upB        : upper bound for integration (MeV)
    Sommerfeld : if True, use O(T^2) Sommerfeld correction
    ...
    def _eps0() -> float:
        if muQ <= me:
            return 0.0
        if me == 0.0:
            return muQ**4 / (4.0 * np.pi**2)
        kF = _kF(muQ, me)
        return ((2.0*kF**3 + me*me*kF)*muQ - me**4*np.log((kF + muQ) / max(me, 1e-300))) / (8.0*np.pi**2)
    ...
    def _eps_int(Tloc: float) -> float:
        def integrand(k):
            Ek = _Ek(k, me)
            z1 = np.clip((Ek - muQ) / Tloc, -700, 700)
            z2 = np.clip((Ek + muQ) / Tloc, -700, 700)
            f = 1.0 / (1.0 + np.exp(z1))
            fbar = 1.0 / (1.0 + np.exp(z2))
...
def neutrino_edens(T, mu_nu, up=4000, Sommerfeld=False):
    """
    Neutrino energy density (massless, single-spin state).
    ...
    if Sommerfeld:
        if T < 0.0:
            raise RuntimeError("negative temperature")
        eps0 = mu_nu**4 / (8.0 * np.pi**2)
        corr = 0.25 * mu_nu * mu_nu * T * T
        return eps0 + corr
    ...
    def _eps_int(Tloc: float) -> float:
        def integrand(k):
            z1 = np.clip((k - mu_nu) / Tloc, -700, 700)
            z2 = np.clip((k + mu_nu) / Tloc, -700, 700)
            f = 1.0 / (1.0 + np.exp(z1))
            fbar = 1.0 / (1.0 + np.exp(z2))
            return k**3 * (f + fbar)
        try:
...
def nucleon_edens(T, mu, sigma, w0, r03, BI, Sommerfeld=False):
    """
    Energy density of a baryon species in RMF.

    Parameters:
    T          : temperature (MeV)
    mu         : baryon chemical potential (MeV)
    sigma, w0  : scalar/vector mean fields
    r03        : isovector mean field
    BI         : baryon index
    Sommerfeld : if True, use O(T^2) Sommerfeld correction
    ...
    def _eps0() -> float:
        ...
        kF = np.sqrt(max(mu_eff * mu_eff - m_eff * m_eff, 0.0))
        Ef = np.sqrt(kF * kF + m_eff * m_eff)
        term1 = (2.0 * kF**3 + m_eff * m_eff * kF) * Ef
        term2 = m_eff**4 * np.log((kF + Ef) / max(m_eff, 1e-300))
        return (term1 - term2) / (8.0 * np.pi**2)
    ...
    def _eps_int(Tloc: float) -> float:
        def integrand(k):
            E = np.sqrt(k * k + m_eff * m_eff)
            z1 = np.clip((E - mu_eff) / Tloc, -700, 700)
            z2 = np.clip((E + mu_eff) / Tloc, -700, 700)
            f = 1.0 / (1.0 + np.exp(z1))
            fbar = 1.0 / (1.0 + np.exp(z2))
            return k * k * E * (f + fbar)
        try:
...
def RMFpressure(nbext, Trmf, para, sigma_init=30, w0_init=20, r03_init=-3, mub_init=990, mue_init=50, verb=False, add_photons=False):
    ...
    P_elec = electron_pressure(Trmf, mue)
    P_photon = _photon_pressure(Trmf) if add_photons else 0

    if max(abs(r) for r in chk) > 1e-6:
        print("Accuracy not achieved, check functions > 10^(-6)")
    
    return meson_pressure + nucpress + P_elec + P_photon + P_offset

def RMFbaryon_density(input_num, input_type, Trmf, para, sigma_init=30, w0_init=20, r03_init=-3, mub_init=990, mue_init=50, verb=False, return_species=False):
    """
    Compute total baryon density for beta-equilibrated charge-neutral npe matter.
    ...
    vevs = _solve_from_input_type(
        input_num,
        input_type,
        lambda nb: RMFsolve(nb, Trmf, para, sigma_init, w0_init, r03_init, mub_init, mue_init, verb),
        lambda mub: RMFsolve_mu(mub, Trmf, para, sigma_init, w0_init, r03_init, mue_init, verb),
    )
    if vevs is None:
        return None
    ...
    if verb and max(abs(r) for r in chk) > 1e-6:
        print("Accuracy not achieved, check functions > 10^(-6)")
...
def RMFedens(nbext, Trmf, para, sigma_init=30, w0_init=20, r03_init=-3, mub_init=990, mue_init=50, verb=False, add_photons=False, use_entropy=False):
    """
    Solve RMF for charge-neutral npe matter and compute total energy density.

    Parameters mirror RMFpressure; energy density is returned in MeV^4.
    """
    _set_couplings(para)
...
def RMFbaryon_densitySYM(input_num, input_type, Trmf, para, sigma_init=30, w0_init=20, mub_init=50, verb=False, return_species=False):
    """
    Compute total baryon density for symmetric nuclear matter (np).
    ...
    vevs = _solve_from_input_type(
        input_num,
        input_type,
        lambda nb: RMFsolveSYM(nb, Trmf, para, sigma_init, w0_init, mub_init, verb),
        lambda mub: RMFsolveSYM_mu(mub, Trmf, para, sigma_init, w0_init, verb),
    )
    if vevs is None:
        return None
...
def RMFedensSYM(input_num, input_type, Trmf, para, sigma_init=30, w0_init=20, mub_init=50, verb=False, renorm=False, use_entropy=False):
    """
    Compute total energy density for symmetric nuclear matter (np).

    Parameters mirror RMFpressureSYM; energy density is returned in MeV^4.
    """
    vevs = _solve_from_input_type(
        input_num,
        input_type,
...
def RMFbaryon_densityPNM(input_num, input_type, Trmf, para, sigma_init=30, w0_init=20, r03_init=-3, mub_init=990, verb=False, return_species=False):
    """
    Compute total baryon density for pure neutron matter (PNM).
...
def RMFedensPNM(input_num, input_type, Trmf, para, sigma_init=30, w0_init=20, r03_init=-3, mub_init=990, verb=False, renorm=False, use_entropy=False):
    """
    Compute total energy density for pure neutron matter (PNM).
...
def pressure_RMF(soltab, add_photons=False, renorm=False, electrons=True, neutrinos=False):
    ...
    if len(soltab[0]) > 6 and isinstance(soltab[0][6], (float, int, np.floating)):
        mu_nu = soltab[0][6]
    else:
        mu_nu = 0.0
    ...
    # Photon pressure
    P_photon = _photon_pressure(T) if add_photons else 0.0
...
def baryon_density_RMF(soltab, return_species=False):
    """
    Compute total baryon density from an RMF solution table.

    Parameters:
    - soltab: RMF solver output table
    - return_species: if True, also return {"n_n": ..., "n_p": ...}
    """
    n_tot, n_n, n_p = _species_baryon_densities_from_soltab(soltab)
    if return_species:
        return n_tot, {"n_n": n_n, "n_p": n_p}
    return n_tot
