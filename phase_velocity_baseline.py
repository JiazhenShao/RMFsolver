...
from scipy.integrate import quad, quad_vec, solve_bvp, solve_ivp
from scipy.optimize import fsolve, root_scalar, root, least_squares, minimize_scalar
...
# Public functions
__all__ = ["P_f", "E_f", "n_B", "nB_QM", "nK_QM", "nQM_em", "PQM", "PQM_em", "edensQM", "entropyQM", "uN", "solve_steady_front_2d", "solve_steady_front_2d_simple", "solve_steady_front_1d_aQstar", "solve_steady_front_1d_aQstar_rescaled", "solve_steady_front_1d_aQstar_rescale_bvp", "solve_steady_front_entropy", "solve_steady_front_entropy_TQguess_uagrad", "extract_jB_curve_vs_aQstar", "vNtoQ_Pc", "vNtoQ_B", "vNtoQ_nc", "Get_Delta_n_max",
...
def _fermion_thermo_state_cached(mu, m, Tem, upB):
    ...
    The finite-T massive branch evaluates all thermodynamic quantities and the
    susceptibility in one vectorized quadrature and reconstructs P from
        P = mu * n + T * s - E.
    """
    mu = float(mu)
    m = float(m)
...
def _solve_steady_front_entropy_TQguess_once(
    ...
    profile_guess=None,
    seed_profile=False,
    entropy_flux_equation="reaction",
):
    """
    Solve the entropy-enabled steady-front problem as a one-shot compact-coordinate BVP
    ...
    if max_nodes <= int(n_mesh):
        raise RuntimeError("max_nodes must be larger than n_mesh")
    if entropy_flux_equation not in ("reaction", "ua_gradient"):
        raise RuntimeError("entropy_flux_equation must be 'reaction' or 'ua_gradient'")
    ...
    def _ode(s_coord, y, p):
        ...
        for i in range(y.shape[1]):
            ...
            try:
                ...
                dyds[1, i] = reaction * dx_ds
                if entropy_flux_equation == "reaction":
                    dw_dx = float((state["nB_Q"] / thermo_loc["T"]) * thermo_loc["muK"] * reaction)
                else:
    ...
    result = {
        ...
        "aQstar": float(aQstar),
        "branch_label": "muK-rich",
        "entropy_flux_equation": entropy_flux_equation,
        "coordinate": "BVP: s_coord in [0, 1-tail_eps], s_coord=1-exp(-lambda*x)",
        "tail_eps": float(tail_eps),
        "u_N": float(state["u_N"]),
        "u_Q": float(state["u_Q"]),
...
def solve_steady_front_entropy_TQguess(
    ...
    return_profile=False,
    verb=False,
    _entropy_flux_equation="reaction",
):
    """
    Solve the entropy-enabled steady-front problem as a compact-coordinate BVP
    ...
    direct_result = _solve_steady_front_entropy_TQguess_once(
        ...
        verb=verb,
        profile_guess=None,
        entropy_flux_equation=_entropy_flux_equation,
    )
    if bool(direct_result.get("success")) or abs_target <= 1.0e-2:
        result_out = dict(direct_result)
        result_out["continuation_used"] = False
...
def solve_steady_front_entropy_TQguess_uagrad(
    T,
    nB_N,
    B_one_forth,
    aQstar,
    ...
    """
    Solve the entropy-enabled steady-front BVP with the entropy equation

        0 = d_x(s*u) + (nB_Q/T) * muK * d_x(u*a).

    This uses the same compact-coordinate BVP and continuation strategy as
    solve_steady_front_entropy_TQguess, with downstream T_Q still promoted to
    a global unknown. The local derivative d_x(u*a) is evaluated from the
    thermodynamic closure u = u(a, w), w = s*u, by finite differencing
    u(a,w)*a in (a,w).
    """
    return solve_steady_front_entropy_TQguess(
        T,
        nB_N,
        B_one_forth,
        aQstar,
        ...
        return_profile=return_profile,
        verb=verb,
        _entropy_flux_equation="ua_gradient",
    )
...
def _solve_shifted_surface_state(P_crit, DelP, B_SQM, T, m_s, param, NM_type, aQmax=True, star_pressure_target=None, upB=5000):
    """
    Solve the common shifted-isobar QM/NM state used by the velocity and
    Delta/a-parameter wrappers.
    """
    if star_pressure_target is None:
        star_pressure_target = float(P_crit + DelP)

    muB_N = _solve_scalar_root(
        lambda x: PNM(x, T, param, NM_type) - P_crit - DelP,
        1050.0,
        xtol=1e-6,
        maxfev=400,
    )
    muB_Q = _solve_scalar_root(
        lambda x: PQM(x, 0.0, B_SQM, T, m_s) - P_crit - DelP,
        1050.0,
        xtol=1e-6,
        maxfev=400,
    )

    q_equil = _quark_uds_state(muB_Q, 0.0, T, ms=m_s, upB=upB)
    nB_Q = float(q_equil["nB"])
    nK_Q = float(q_equil["nK"])
    chiK_Q = float(q_equil["susceptibilities"]["chiK"])

    if nB_Q <= 0.0:
        raise RuntimeError("Equilibrated quark state has non-positive baryon density")

    if aQmax:
        muB_star, muK_star = _find_muB_muK_star(PQM, P_crit + DelP, muB_N, B_SQM, T, m_s, upB=upB)
    else:
        if chiK_Q <= 0.0:
            raise RuntimeError("Equilibrated quark kaon susceptibility must be positive")
        delta_mu = float(muB_N - muB_Q)
        if delta_mu < 0.0:
            raise RuntimeError("Shifted state produced muB_N < muB_Q")
        muK_star = float(np.sqrt(max(0.0, 2.0 * nB_Q * delta_mu / chiK_Q)))
        muB_star = _solve_scalar_root(
            lambda x: PQM(x, muK_star, B_SQM, T, m_s) - star_pressure_target,
            muB_Q,
            xtol=1e-6,
            maxfev=400,
        )

    q_star = _quark_uds_state(muB_star, muK_star, T, ms=m_s, upB=upB)
    aQstar = float((q_star["nK"] - nK_Q) / nB_Q)

    nB_N = float(nB_NM_type(muB_N, T, param=param, NM_type=NM_type))
    aN = float((nB_N - nK_Q) / nB_Q)

    return {
        "muB_N": float(muB_N),
        "muB_Q": float(muB_Q),
        "muB_star": float(muB_star),
        "muK_star": float(muK_star),
        "nB_N": nB_N,
        "nB_Q": nB_Q,
        "nK_Q": nK_Q,
        "chiK_Q": chiK_Q,
...
def vNtoQ_Pc(T, P_crit, DelP, m_s, param, NM_type, method="analytical", aQmax=True):
    ...
    muB_crit = _solve_scalar_root(
        lambda x: PNM(x, T, param, NM_type) - P_crit,
        1050.0,
        xtol=1e-6,
        maxfev=400,
    )
    B_SQM = _solve_scalar_root(
        lambda x: PQM(muB_crit, 0.0, x, T, m_s) - P_crit,
        180.0,
        xtol=1e-6,
        maxfev=400,
    )
    surface = _solve_shifted_surface_state(
        P_crit,
        DelP,
        B_SQM,
        T,
        ...
        NM_type,
        aQmax=aQmax,
        star_pressure_target=P_crit + DelP,
    )
    aQstar = surface["aQstar"]
    aN = surface["aN"]
    muB_star = surface["muB_star"]
...
def vNtoQ_Pc_fixB(T, P_crit0, DelP, m_s, param, NM_type, method="analytical", aQmax=True):
    ...
    muB_crit0 = _solve_scalar_root(
        lambda x: PNM(x, Temp=0, param=param, NM_type=NM_type) - P_crit0,
        1050.0,
        xtol=1e-6,
        maxfev=400,
    )
    B_SQM = _solve_scalar_root(
        lambda x: PQM(muB_crit0, 0.0, x, 0.0, m_s) - P_crit0,
        180.0,
        xtol=1e-6,
        maxfev=400,
    )
    muB_crit = _solve_scalar_root(
        lambda x: PNM(x, T, param, NM_type) - PQM(x, 0.0, B_SQM, T, m_s),
        1050.0,
        xtol=1e-6,
        maxfev=400,
    )
    P_crit = float(PQM(muB_crit, 0.0, B_SQM, T, m_s))
    surface = _solve_shifted_surface_state(
        P_crit,
        DelP,
        B_SQM,
...
def vNtoQ_B(T, B_SQM, DelP, m_s, param, NM_type, method="analytical", aQmax=True):
    ...
    muB_crit = _solve_scalar_root(
        lambda x: PQM(x, 0.0, B_SQM, T, m_s) - PNM(x, T, param, NM_type),
        1050.0,
        xtol=1e-6,
        maxfev=400,
    )
...
def vNtoQ_nc(T, n_crit, Deln, m_s, param, NM_type, method="analytical", aQmax=True):
    ...
    DelP = float(PNM_n(n_crit + Deln, T, param, NM_type) - P_crit)
    surface = _solve_shifted_surface_state(
        P_crit,
        DelP,
        B_SQM,
...
def vNtoQ_fixB(T, n_crit, Deln, m_s, param, NM_type, method="analytical", aQmax=True):
    ...
    P_crit0 = PNM_n(n_crit, Temp=0.0, param=param, NM_type=NM_type)
    muB_crit0 = _solve_scalar_root(
        lambda x: PNM(x, Temp=0.0, param=param, NM_type=NM_type) - P_crit0,
        1050.0,
        xtol=1e-6,
        maxfev=400,
    )
    ...
    P_crit = float(PQM(muB_crit, 0.0, B_SQM, T, m_s))
    n_crit_T = _solve_scalar_root(
        lambda x: PNM_n(x, T, param, NM_type) - P_crit,
        2.0 * n0,
        xtol=1e-6,
        maxfev=400,
    )
    DelP = float(PNM_n(n_crit_T + Deln, T, param, NM_type) - P_crit)
...
def Get_Delta_n_max(T, n_crit, m_s, param, NM_type, aQmax=True, tol=5e-3, coarse_pts=5, bounds=None):
    ...
    def Get_aN_aQstar_diff(Delta_n):
        DelP = float(PNM_n(n_crit + Delta_n, T, param, NM_type) - P_crit)
        surface = _solve_shifted_surface_state(
            P_crit,
            DelP,
            B_SQM,
